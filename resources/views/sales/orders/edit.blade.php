@extends('layouts.app')
@section('title', 'Edit Sales Order - ' . $order->so_number)
...
<div class="animate-in" x-data="soForm()">
    <div class="page-header">
        <div>
            <h1>Edit Sales Order: {{ $order->so_number }}</h1>
            <p>Perbarui rincian Sales Order yang berstatus Draft</p>
        </div>
        <div style="display:flex; gap:8px;">
            ...
            <a href="{{ route('sales.orders.index') }}" class="btn btn-secondary">
                <i class="fa-solid fa-list"></i> Daftar SO
            </a>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('sales.orders.update', $order) }}">
            ...
            <div class="card" style="grid-column:span 2;">
                ...
                <div class="card-body">
                    <div class="form-row form-row-2">
                        <div class="form-group">
                            ...
                            <select name="customer_id" class="form-control {{ $errors->has('customer_id') ? 'is-invalid' : '' }}" required>
                                ...
                                <option value="{{ $c->id }}" {{ old('customer_id', $order->customer_id) == $c->id ? 'selected' : '' }}>{{ $c->name }} ({{ $c->code }})</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" name="expected_delivery_date" value="{{ old('expected_delivery_date', $order->expected_delivery_date ? $order->expected_delivery_date->format('Y-m-d') : '') }}" class="form-control">
                        </div>
                        <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="notes" class="form-control" rows="2" placeholder="Catatan SO...">{{ old('notes', $order->notes) }}</textarea>
                    </div>
                </div>
            </div>
        ...
        <div style="display:flex; justify-content:flex-end; gap:12px;">
            <a href="{{ route('sales.orders.show', $order) }}" class="btn btn-secondary">Batal</a>
            <button type="submit" class="btn btn-primary">
                <i class="fa-solid fa-floppy-disk"></i> Perbarui Sales Order
            </button>
        </div>
    </form>
</div>

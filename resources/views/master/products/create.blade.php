            ...
            <form method="POST" action="{{ isset($product) ? route('master.products.update', $product) : route('master.products.store') }}" enctype="multipart/form-data">
                ...
                <div class="form-row form-row-2">
                    <div class="form-group">
                        ...
                        @if(!$errors->has('sku'))
                        <span class="form-text" style="font-size:11px; color:var(--text-secondary); margin-top:3px;">Kode pengenal unik. Tidak boleh sama dengan produk lain.</span>
                        @endif
                        @error('sku')
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="unit" value="{{ old('unit', $product->unit ?? '') }}" class="form-control {{ $errors->has('unit') ? 'is-invalid' : '' }}" required placeholder="pcs, box, kg...">
                        @error('unit')
                        <div class="invalid-feedback" style="display:flex; align-items:flex-start; gap:6px; color:#b91c1c; font-size:12px; margin-top:6px; background:#fef2f2; padding:8px 10px; border-radius:6px; border:1px solid #fecaca; line-height:1.4;">
                            <i class="fa-solid fa-circle-exclamation" style="margin-top:2px; flex-shrink:0;"></i>
                            <span>{{ $message }}</span>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="name" value="{{ old('name', $product->name ?? '') }}" class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" required placeholder="Contoh: Keyboard Mechanical Keychron K2">
                    @error('name')
                    <div class="invalid-feedback" style="display:flex; align-items:flex-start; gap:6px; color:#b91c1c; font-size:12px; margin-top:6px; background:#fef2f2; padding:8px 10px; border-radius:6px; border:1px solid #fecaca; line-height:1.4;">
                        <i class="fa-solid fa-circle-exclamation" style="margin-top:2px; flex-shrink:0;"></i>
                        <span>{{ $message }}</span>
                ...
                <div class="form-group" x-data="{ 
                    ...
                    <div style="display:flex; gap:16px; align-items:center;">
                        <!-- Preview Box -->
                        <div style="width:90px; height:90px; border-radius:8px; border:2px dashed #cbd5e1; background:#f8fafc; display:flex; align-items:center; justify-content:center; overflow:hidden; position:relative; flex-shrink:0;">
                            <template x-if="previewUrl">
                                <div style="width:100%; height:100%; position:relative;">
                                    <img :src="previewUrl" alt="Preview Foto" style="width:100%; height:100%; object-fit:cover;">
                                    <button type="button" @click="clearFile()" style="position:absolute; top:4px; right:4px; background:rgba(220, 38, 38, 0.85); color:#fff; border:none; border-radius:50%; width:20px; height:20px; display:flex; align-items:center; justify-content:center; cursor:pointer; font-size:10px;" title="Hapus foto">
                            ...
                            <template x-if="!previewUrl">
                                <div style="text-align:center; color:#94a3b8;">
                                    <i class="fa-solid fa-image" style="font-size:24px; margin-bottom:2px; display:block;"></i>
                                    <span style="font-size:10px;">Foto Produk</span>
                                </div>
                            </template>
                        </div>
                        ...
                        <!-- File Input Controls -->
                        <div style="flex:1;">
                            <input type="file" name="image" x-ref="fileInput" @change="handleFileChange($event)" accept="image/jpeg,image/png,image/jpg,image/webp,image/svg+xml" class="form-control {{ $errors->has('image') ? 'is-invalid' : '' }}" style="padding:6px 12px; font-size:13px;">
                            <div style="font-size:11px; color:var(--text-secondary); margin-top:4px;">
                                Format yang didukung: JPG, PNG, WEBP, SVG (Maks. 3MB).
                            </div>
                            @error('image')
                            <div class="invalid-feedback" style="display:flex; align-items:flex-start; gap:6px; color:#b91c1c; font-size:12px; margin-top:6px; background:#fef2f2; padding:8px 10px; border-radius:6px; border:1px solid #fecaca; line-height:1.4;">
                                <i class="fa-solid fa-circle-exclamation" style="margin-top:2px; flex-shrink:0;"></i>

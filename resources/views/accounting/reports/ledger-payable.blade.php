...
@section('title', 'Kartu Hutang - ' . $supplier->name)
    ...
    <div class="page-header" style="margin-bottom: 20px;">
        <div>
            <h1 style="font-size: 22px; font-weight: 700; color: var(--text-primary); margin-bottom: 4px;">
                ...
                Kartu Hutang: {{ $supplier->name }}
            </h1>
            <p style="color: var(--text-secondary); font-size: 13px; margin: 0;">
                Buku Pembantu Hutang Usaha (Subsidiary Ledger) · Kode Supplier: <strong style="color: var(--primary);">{{ $supplier->code }}</strong>
            </p>
        </div>
        <div style="display: flex; gap: 8px;">
            <a href="{{ route('accounting.reports.payables-by-vendor') }}" class="btn btn-secondary btn-sm">
                <i class="fa-solid fa-arrow-left"></i> Rekap Vendor
            </a>
            <a href="{{ route('master.suppliers.show', $supplier) }}" class="btn btn-secondary btn-sm">
                <i class="fa-solid fa-building"></i> Detail Supplier
            </a>
    ...
    <div class="grid grid-4" style="gap: 16px; margin-bottom: 20px;">
        <div class="card" style="border-left: 4px solid #64748b; padding: 16px 20px;">
            ...
            <div style="font-size: 11.5px; color: var(--text-secondary); margin-top: 4px;">
                Saldo hutang sebelum periode filter
            </div>
        </div>
        ...
        <div class="card" style="border-left: 4px solid #10b981; padding: 16px 20px;">
            ...
            <div style="font-size: 11.5px; color: #059669; margin-top: 4px; font-weight: 500;">
                <i class="fa-solid fa-circle-check"></i> Pembayaran & Retur Beli
            </div>
        </div>
        ...
        <div class="card" style="border-left: 4px solid var(--primary); padding: 16px 20px;">
            ...
            <div style="font-size: 22px; font-weight: 700; color: {{ $endingBalance > 0 ? '#dc2626' : '#059669' }}; margin-top: 4px;">
                Rp {{ number_format($endingBalance, 0, ',', '.') }}
            </div>
            <div style="font-size: 11.5px; color: var(--text-secondary); margin-top: 4px;">
                Sisa kewajiban hutang berjalan
            </div>
        </div>
    </div>

    {{-- Filter Rentang Tanggal --}}
    <form method="GET" action="{{ route('accounting.reports.ledger-payable', $supplier) }}" class="card" style="padding: 14px 20px; margin-bottom: 20px; display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 12px;">
        <div style="display: flex; align-items: center; gap: 12px; flex-wrap: wrap;">
            ...
            @if($dateFrom || $dateTo)
            <a href="{{ route('accounting.reports.ledger-payable', $supplier) }}" class="btn btn-secondary btn-sm" style="height: 36px; padding: 0 12px;" title="Reset Filter">
                Reset
            </a>
            @endif
        </div>
            ...
            <table class="erp-table" style="margin: 0; width: 100%;">
                <thead>
                    <tr>
                        ...
                        <th>Keterangan</th>
                        <th style="text-align: right; width: 140px; color: #dc2626;">Tambah Hutang (+)</th>
                        <th style="text-align: right; width: 140px; color: #059669;">Kurang Hutang (-)</th>
                ...
                <tbody>
                    ...
                    <tr style="background: #f8fafc; font-weight: 600; border-bottom: 2px solid #e2e8f0;">
                        ...
                        <td style="color: var(--text-secondary);">
                            Saldo Awal Hutang {{ $dateFrom ? 'per ' . date('d/m/Y', strtotime($dateFrom)) : 'Awal Pembukuan' }}
                        </td>
                        <td style="text-align: right; color: var(--text-secondary);">-</td>
                        <td style="text-align: right; color: var(--text-secondary);">-</td>
                        <td style="text-align: right; font-weight: 700; color: {{ $beginningBalance > 0 ? '#dc2626' : 'var(--text-primary)' }};">
                            Rp {{ number_format($beginningBalance, 0, ',', '.') }}
                        </td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td style="text-align: right; font-weight: 600; color: {{ $t->credit > 0 ? '#dc2626' : 'var(--text-secondary)' }};">
                            {{ $t->credit > 0 ? 'Rp ' . number_format($t->credit, 0, ',', '.') : '-' }}
                        </td>
                        <td style="text-align: right; font-weight: 600; color: {{ $t->debit > 0 ? '#059669' : 'var(--text-secondary)' }};">
                            {{ $t->debit > 0 ? 'Rp ' . number_format($t->debit, 0, ',', '.') : '-' }}
                        </td>
                        <td style="text-align: right; font-weight: 700; color: {{ $t->running_balance > 0 ? '#dc2626' : '#059669' }};">
                    ...
                    <tr>
                        <td colspan="8" style="text-align: center; padding: 28px; color: var(--text-secondary);">
                            ...
                            Tidak ada transaksi hutang pada periode filter ini.
                        </td>
                    </tr>
                    @endforelse
                </tbody>
                <tfoot>
                    <tr style="background: #f1f5f9; font-weight: 700; border-top: 2px solid #cbd5e1;">
                        ...
                        <td style="text-align: right; color: #dc2626;">Rp {{ number_format($totalCredit, 0, ',', '.') }}</td>
                        <td style="text-align: right; color: #059669;">Rp {{ number_format($totalDebit, 0, ',', '.') }}</td>
                        <td style="text-align: right; color: {{ $endingBalance > 0 ? '#dc2626' : '#059669' }}; font-size: 14px;">
                            Rp {{ number_format($endingBalance, 0, ',', '.') }}
                        </td>
                    </tr>
                    <tr style="background: #ffffff; font-size: 12px; color: var(--text-secondary);">
                        <td colspan="8" style="padding: 10px 16px; text-align: right;">
                            Formula: <strong>Saldo Akhir</strong> = Saldo Awal (Rp {{ number_format($beginningBalance, 0, ',', '.') }}) + Penambahan (Rp {{ number_format($totalCredit, 0, ',', '.') }}) − Pengurangan (Rp {{ number_format($totalDebit, 0, ',', '.') }}) = <strong>Rp {{ number_format($endingBalance, 0, ',', '.') }}</strong>
                        </td>
                    </tr>
                </tfoot>
            </table>
